...
$start_date = old('start_date') ?? ($start_date ?? date('d.m.Y'));
$end_date = old('end_date') ?? ($end_date ?? date('d.m.Y'));
$start_time = old('start_time') ?? ($start_time ?? null);
$end_time = old('end_time') ?? ($end_time ?? null);
$category = old('category') ?? ($category ?? '');
$tags = old('tags') ?? ($tags ?? 'myCalendar');
$status = old('status') ?? ($status ?? '2');
$message_status = old('message_status') ?? ($message_status ?? '1');
$subscribe_status = old('subscribe_status') ?? ($subscribe_status ?? '1');
$content = html_entity_decode(old('content') ?? ($content ?? ''));
?>
<div class="row">
    ...
    <div class="col-lg-8 mb-4">
        ...
        <input type="text" class="form-control" placeholder="Başlık.." name="title" value="<?= $title ?>"
               required>
    </div>
    ...
    <div class="col-lg-6 mb-4">
        ...
        <input type="text" id="start_dt" class="form-control" data-input="daterange" name="date" value="<?=$start_date?> - <?=$end_date?>" required>
    ...
    <div class="col-lg-3 mb-4">
        ...
        <div class="input-group clockpicker">
            <input type="text" id="start_time" class="form-control" name="start_time"
                   value="<?= $start_time ?? date('H:i') ?>" required>
    ...
    <div class="col-lg-3 mb-4">
        ...
        <div class="input-group clockpicker">
            <input type="text" id="end_time" class="form-control" name="end_time"
                   value="<?= $end_time ?? date('H:i', strtotime('+15 minutes')) ?>" required>
            <span class="input-group-addon">
                <span class="glyphicon glyphicon-time"></span>
            </span>
        </div>
    ...
    <div class="form-group col-md-4">
        ...
        <div class="w-100 custom-control custom-radio custom-radio-color-checked custom-control-inline mb-1">
            <input type="radio" class="custom-control-input bg-success" id="public"
                   name="status"
                   value="2" <?= $status == '2' ? 'checked' : '' ?> required>
            <label class="custom-control-label"
